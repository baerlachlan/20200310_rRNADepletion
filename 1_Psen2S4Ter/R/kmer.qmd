...
```{r}
rrna_genes <- genesGR_Dr %>%
    plyranges::filter(str_detect(gene_biotype, "rRNA")) %>%
    as_tibble() %>%
    pull(gene_id)
```

```{r}
enframe(kappa_k12, name = "gene_id", value = "kappa") %>%
    dplyr::arrange(abs(kappa)) %>%
    with(which(gene_id %in% rrna_genes))
```
