...
## Jellyfish
JELLYFQ=${PROJROOT}/6_jellyfish2pass
...
for k in $(seq 20 20)
  do
    ...
    for R1 in ${STAR2}/fastq/*R1.fastq.gz
      do

        R2=${R1%_R1.fastq.gz}_R2.fastq.gz
        echo -e "Counting ${k}-mers in R1 file:\t${R1}"
        echo -e "Also counting ${k}-mers in R2 file:\t${R2}"
        ...
        /usr/bin/jellyfish count -m ${KMER} -s 2G --bf-size 2G -C -t ${CORES} -o ${out}_counts.jf <(zcat ${R1}) <(zcat ${R2})
