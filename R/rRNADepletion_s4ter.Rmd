...
```{r counts}
dgeList <- read_tsv("/hpcfs/users/a1647910/20200310_rRNADepletion/1_Psen2S4Ter/2_alignedData/featureCounts/genes.out") %>%
...
covTest <- function(goi){
  ...
  test <- lapply(covList, function(x){
    ...
    left_join(dgeList$samples[,c("sample", "rRNA")], by = "sample") %>%
    with(
...
```{r}
covRes <- mclapply(deUp, covTest, mc.cores = 6) 
covRes %>% 
  purrr::reduce(rbind) %>%
  dplyr::arrange(P) %>%
  left_join(k8overlapUU, by = "Geneid") %>%
  dplyr::select(Geneid, Symbol, -Length, -Overlap, -Norm, adj.r.squared, P) %>%
  mutate(
    adjP = p.adjust(P, "fdr"),
    sig = adjP < 0.05
  ) %>% print(n = 50)
upGenes <- topTable %>% dplyr::filter(logFC > 0) %>% .$Geneid
upTest <- lapply(upGenes, covTest)
```
