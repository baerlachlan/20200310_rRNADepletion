...
topRes(k5topTable,
       cap = paste(
         ...
         "of",
         nrow(k5topTable),
         "detected sequences were classified as DE with an Bonferroni p-value < 0.05."
       )
)
```
...
```{r, fig.cap="*Volcano plot showing -log10(p-value) against logFC. Kmers classified as DE with Bonferroni p-value < 0.05 are highlighted in red.*"}
k5topTable %>%
  ...
  geom_text_repel(
    data = . %>%
      ...
      dplyr::filter(DE | -log10(P.Value) > 4 | logFC > 5 | logFC < -2.5),
...
k6topTable %>%
  ...
  geom_text_repel(
    data = . %>%
      dplyr::filter(-log10(P.Value) > 2.5 | logFC > 7 | logFC < -2.5),
    aes(label = mer, color = "black")
  ) +
...
k7topTable %>%
  ...
  geom_text_repel(
    data = . %>%
      dplyr::filter(DE | -log10(P.Value) > 5.5 | logFC > 16 | logFC < -5),
    aes(label = mer, color = "black")
  ) +
...
k8topTable %>%
  ...
  geom_text_repel(
    data = . %>%
      dplyr::filter(DE & -log10(P.Value) > 6.5 | logFC > 30),
    aes(label = mer, color = "black")
  ) +
...
k9topTable %>%
  ...
  geom_text_repel(
    data = . %>%
      dplyr::filter(DE & -log10(P.Value) > 7.3 | logFC < -14 | logFC > 45), 
    aes(label = mer, color = "black")
  ) +
...
k10topTable %>%
  ...
  geom_text_repel(
    data = . %>%
      dplyr::filter(DE & -log10(P.Value) > 8 | logFC > 52 | logFC < -17),
    aes(label = mer, color = "black")
  ) +
