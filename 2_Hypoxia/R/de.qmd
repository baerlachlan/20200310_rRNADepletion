...
This is a total RNA-seq dataset generated from a 3-way comparison of WT zebrafish (*Danio rerio*) with heterozygous mutants (psen2^S4Ter/+^) and homozygous mutants (psen2^S4Ter/S4Ter^). A [previous analysis](https://uofabioinformaticshub.github.io/20170327_Hypoxia_RNASeq/index.html) of this dataset identified the possibility of incomplete ribosomal RNA (rRNA) removal. The following analysis involves an investigation into possible reasons for incomplete rRNA removal and any bias this introduces into the data.
...
```{r rRnaProp}
fl <- list.files(
    here("2_Hypoxia/analysis-kmer/results/bwa/rrna_props/"),
    full.names = TRUE
)
rRnaProp <- lapply(fl, read_lines) %>%
    set_names(str_remove_all(basename(fl), "\\d+-MORGAN-|_S.+")) %>%
    # .[!str_detect(names(.), "6Q|24Q")] %>%
    .[!str_detect(names(.), "6Q|24Q|24P")] %>%
    unlist() %>%
    enframe(name = "sample", value = "proportion") %>%
    left_join(samples) %>%
    mutate(proportion = as.numeric(str_remove(proportion, "%$")) / 100)
# rRnaProp <- read.delim(
...
```{r counts}
# dgeList <- read_tsv("/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia/4_star2pass/featureCounts/genes.out") %>%
dgeList <- read_tsv(
    paste0(
        "/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia",
        "/smk-rnaseq-counts-1.2.4/results/featureCounts/reverse/all.featureCounts"
    ),
    comment = "#"
) %>%
...
dgeList$samples <- dgeList$samples %>%
    ...
    left_join(rRnaProp[,c("sample", "proportion")]) %>%
    dplyr::rename(rRNA = proportion)
# dgeList$samples$filenames <- list.files(
#   "/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia/2_alignedData/bam", 
#   pattern = ".bam$", 
#   full.names = TRUE
# )
```
...
dgeFilt <- dgeList[genes2keep,, keep.lib.sizes = FALSE] %>%
    calcNormFactors()
dgeFilt$samples <- dgeFilt$samples %>%
    mutate(group = fct_relevel(group, "normoxia_6m"))
```
...
```{r}
displayRes_de(tt_g$grouphypoxia_6m)
```
...
displayRes_de(tt_gr$grouphypoxia_6m)
```
...
```{r}
saveRDS(addInfo, here("2_Hypoxia/Rdata/addInfo.Rds"))
saveRDS(dgeList, here("2_Hypoxia/Rdata/dgeList.Rds"))
saveRDS(dgeFilt, here("2_Hypoxia/Rdata/dgeFilt.Rds"))
saveRDS(tt_r, here("2_Hypoxia/Rdata/tt_r.Rds"))
saveRDS(tt_g, here("2_Hypoxia/Rdata/tt_g.Rds"))
saveRDS(tt_gr, here("2_Hypoxia/Rdata/tt_gr.Rds"))
```
