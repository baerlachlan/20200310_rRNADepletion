...
```{r loadObjects}
# load(
#     here::here("2_Hypoxia/R/output/1_1_DE.RData")
# )
```
...
```{r load_data}
addInfo <- readRDS(here("2_Hypoxia/Rdata/addInfo.Rds"))
```

```{r}
files <- list.files(
    # path = "/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia/old_bash_analysis/0_rawData/FastQC",
...
samples <- tibble(
    ...
    mutate(
        sample = str_remove(sample, "_1_R0"),
        dataset = "Hypoxia",
        organism = "zebrafish",
        group_1 = case_when(
    ...
    dplyr::filter(str_detect(sample, "6P"))
...
```{r k5counts}
k5files <- list.files(
    "/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia/analysis-kmer/results/jellyfish/k5",
    pattern = ".dumps", full.names = TRUE
)
k5counts <- lapply(k5files, function(x){
    read_delim(x, col_names = c("mer", basename(x)), delim = " ") %>%
        set_colnames(str_remove_all(colnames(.), "\\d+-MORGAN-|_S\\d+_R1_001.dumps"))
}) %>%
    purrr::reduce(full_join) %>%
    as.data.frame() %>%
    column_to_rownames("mer") %>%
    .[,colnames(.) %in% samples$sample] %>%
...
```{r k6counts}
k6files <- list.files(
    "/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia/analysis-kmer/results/jellyfish/k6",
    pattern = ".dumps", full.names = TRUE
)
...
```{r k7counts}
k7files <- list.files(
    "/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia/analysis-kmer/results/jellyfish/k7",
    pattern = ".dumps", full.names = TRUE
)
...
```{r k8counts}
k8files <- list.files(
    "/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia/analysis-kmer/results/jellyfish/k8",
    pattern = ".dumps", full.names = TRUE
)
...
```{r k9counts}
k9files <- list.files(
    "/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia/analysis-kmer/results/jellyfish/k9",
    pattern = ".dumps", full.names = TRUE
)
...
```{r k10counts}
k10files <- list.files(
    "/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia/analysis-kmer/results/jellyfish/k10",
    pattern = ".dumps", full.names = TRUE
)
...
```{r k11counts}
k11files <- list.files(
    "/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia/analysis-kmer/results/jellyfish/k11",
    pattern = ".dumps", full.names = TRUE
)
...
```{r k12counts}
k12files <- list.files(
    "/mnt/phoenix/hpcfs/20200310_rRNADepletion/2_Hypoxia/analysis-kmer/results/jellyfish/k12",
    pattern = ".dumps", full.names = TRUE
)
...
```{r}
saveRDS(k5counts, here("2_Hypoxia/Rdata/k5counts.Rds"))
saveRDS(k6counts, here("2_Hypoxia/Rdata/k6counts.Rds"))
saveRDS(k7counts, here("2_Hypoxia/Rdata/k7counts.Rds"))
saveRDS(k8counts, here("2_Hypoxia/Rdata/k8counts.Rds"))
saveRDS(k9counts, here("2_Hypoxia/Rdata/k9counts.Rds"))
saveRDS(k10counts, here("2_Hypoxia/Rdata/k10counts.Rds"))
saveRDS(k11counts, here("2_Hypoxia/Rdata/k11counts.Rds"))
saveRDS(k12counts, here("2_Hypoxia/Rdata/k12counts.Rds"))
saveRDS(k5topTable, here("2_Hypoxia/Rdata/k5topTable.Rds"))
saveRDS(k6topTable, here("2_Hypoxia/Rdata/k6topTable.Rds"))
saveRDS(k7topTable, here("2_Hypoxia/Rdata/k7topTable.Rds"))
saveRDS(k8topTable, here("2_Hypoxia/Rdata/k8topTable.Rds"))
saveRDS(k9topTable, here("2_Hypoxia/Rdata/k9topTable.Rds"))
saveRDS(k10topTable, here("2_Hypoxia/Rdata/k10topTable.Rds"))
saveRDS(k11topTable, here("2_Hypoxia/Rdata/k11topTable.Rds"))
saveRDS(k12topTable, here("2_Hypoxia/Rdata/k12topTable.Rds"))
```
